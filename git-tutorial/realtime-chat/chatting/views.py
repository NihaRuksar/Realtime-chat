...
def chat_view(request, chatroom_name='public-chat'):
    ...
    # Private chat: only allow members inside
    if chat_group.is_private:
        if request.user not in chat_group.members.all():
            raise Http404()

        # Get the other user (for UI display)
        for member in chat_group.members.all():
            if member != request.user:
                other_user = member
                break

    # Group chat: verify email before joining
    ...
    # HTMX message submit handler
    if request.htmx:
        form = ChatmessageCreateForm(request.POST)
        if form.is_valid():   # <-- FIXED (wrong earlier)
